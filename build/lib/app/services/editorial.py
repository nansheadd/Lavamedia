...
class EditorialWorkflowService:
    ...
    def import_docx_payload(self, data: bytes) -> dict[str, str]:
    ...
    async def translate_content(
        ...
        *,
        ...
        await self._get_content(content_id)  # ensures content exists
        raise RuntimeError("Translation service is not configured yet.")
...
def _extract_docx_payload(data: bytes) -> dict[str, str]:
    try:
        document = Document(BytesIO(data))
    except Exception as exc:  # pragma: no cover - defensive
        raise ValueError("Impossible de lire le fichier DOCX fourni.") from exc
    ...
    title = paragraphs[0]
    chapeau = paragraphs[1] if len(paragraphs) > 1 else ""
    body_parts = paragraphs[2:] if len(paragraphs) > 2 else []
